...
def prune_leaves(t, vals):
    ...
    for branch in branches(t):
        if prune_leaves(branch, vals):
            bran.append(prune_leaves(branch, vals))
    modified = tree(label(t), bran)
    return modified
...
def subseqs(s):
    ...
    if len(s) == 0:
        ...
        subsequences = []
        for subseq in subseqs(s[1:]):
            subsequences.append(subseq)
            subsequences.append([s[0]] + subseq)
        return subsequences
...
def subset_sum(target, lst):
    ...
    for i in lst:
        remainder = lst[:]
        remainder.remove(i)
        if subset_sum(target - i, remainder):
            return True
    return False
