...
def berry_finder(t):
    ...
    if is_leaf(t):
        ...
        branches_find = [berry_finder(a) for a in branches(t)]
        return (sum(branches_find) or False) and True
...
def closer_city(lat, lon, city_a, city_b):
    ...
    distanceA = distance(point, city_a)
    distanceB = distance(point, city_b)
...
def dejavu(t, n):
    ...
    if is_leaf(t):
        ...
        path_find = [dejavu(a, n - label(t)) for a in branches(t)]
        return (sum(path_find) or False) and True
...
def hailstone_tree(n, h):
    ...
    if h >= 1 and (n - 1) % 3 == 0 and (n - 1) // 3 != 1 and n != 1:
