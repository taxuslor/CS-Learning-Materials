...
from networkx import is_digraphical

"""from lab.lab06.lab06 import interval
from lab.lab06.lab06 import lower_bound, upper_bound
from projects.cats.utils import lower"""
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            if (
                ...
                and not place.ant.ant_contained
            ):
                place.ant.ant_contained = self
            elif (
                self.is_container
                and not place.ant.is_container
                and not self.ant_contained
            ):
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        self.attack_range = interval(self.lower_bound, self.upper_bound)
        if self.place:
            next_place = self.place
            index = 0
            while next_place and not next_place.is_hive:
                if (
                    lower_bound(self.attack_range)
                    <= index
                    <= upper_bound(self.attack_range)
                ):
                    if next_place.bees:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        final_damage = reflected_damage
        if self.place:
            if self.health <= amount:
                final_damage += self.damage
            index = 0
            num_bees = len(self.place.bees)
            while index < num_bees:
                bee = self.place.bees[index]
                if final_damage >= bee.health:
                    num_bees -= 1
                else:
                    index += 1
                bee.reduce_health(amount=final_damage)
        Ant.reduce_health(self, amount=amount)
        # END Problem 5
...
# The WallAnt class
class WallAnt(Ant):
    """An ant that does nothing each turn. A WallAnt is useful because it has a large health value."""
...
# The HungryAnt Class
class HungryAnt(Ant):
    """An ant that will eat a random lucky bee. It will take 3 turns to eat one bee."""
    ...
    def action(self, gamestate):
        if self.turns_to_chew:
            ...
            if self.place.bees:
                ...
                lucky_bee.reduce_health(amount=lucky_bee.health)
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 8a
        if not other.is_container and (self.is_container and not self.ant_contained):
            return True
        else:
            return False
        # END Problem 8a
...
class BodyguardAnt(ContainerAnt):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(*args, health, **kwargs)
...
class TankAnt(ContainerAnt):
    """TankAnt provides protection to other Ants, and return offense."""
    ...
    def __init__(self, *args, health=2, **kwargs):
        super().__init__(*args, health, **kwargs)
