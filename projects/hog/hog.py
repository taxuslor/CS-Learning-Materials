...
def boar_brawl(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 2
    exp = 0
    while opponent_score // pow(10, exp):
        exp += 1
    ones = player_score % 10
    tens = (opponent_score % 100 - opponent_score % 10) // 10
    score = 3 * abs(ones - tens)
    if score:
        return score
    else:
        return 1
...
def fuzzy_points(score):
    ...
    if hog_gcd(score, 100) > 10:
        score += 2 * (hog_gcd(score, 100) % 100 - hog_gcd(score, 100) % 10) // 10
    return score
    # END PROBLEM 4
...
def is_always_roll(strategy, goal=GOAL):
    ...
    it0 = 0
    it1 = 0
    roll_numbers = strategy(it0, it1)
    while it0 < goal:
        while it1 < goal:
            if roll_numbers != strategy(it0, it1):
                return False
            it1 += 1
        it0 += 1
        it1 = 0
...
def make_averaged(original_function, total_samples=1000):
    ...
    def f(*args):
        ...
        faces = 0
        while samples != 0:
            faces += original_function(*args)
            samples -= 1
        face = faces / total_samples
        return face
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    rolls_number = 1
    highest_score = 0
    while rolls_number <= 10:
        score = make_averaged(roll_dice, total_samples)(rolls_number, dice)
        if score > highest_score:
            target_number = rolls_number
            highest_score = score
        rolls_number += 1
